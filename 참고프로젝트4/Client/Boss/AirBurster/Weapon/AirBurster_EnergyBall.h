...
/// 에어버스터 전기 투사체
/// </summary>
class CAirBurster_EnergyBall : public CBullet
{
	INFO_CLASS(CAirBurster_EnergyBall, CBullet)

private:
	CAirBurster_EnergyBall(ComPtr<ID3D11Device> pDevice, ComPtr<ID3D11DeviceContext> pDeviceContext);
	CAirBurster_EnergyBall(const CAirBurster_EnergyBall& rhs);
	virtual ~CAirBurster_EnergyBall() = default;
...
public:
	static shared_ptr<CAirBurster_EnergyBall> Create(ComPtr<ID3D11Device> pDevice, ComPtr<ID3D11DeviceContext> pContext);
...
private:
	...
	_float m_fSpeed = { 5.f };
	FTimeChecker	m_fTrailTick = { 0.1f };
