...
SamplerState gSamClampAnisotropic : IMMUTABLE
{
    Filter = ANISOTROPIC;
    AddressU = Clamp;
    AddressV = Clamp;
...
float4 FxaaTexLod0(Texture2D OriginTexture, float2 pos)
{
    return OriginTexture.SampleLevel(gSamClampAnisotropic, pos.xy, 0.0);
}

float4 FxaaTexGrad(Texture2D OriginTexture, float2 pos, float2 grad)
{
    return OriginTexture.SampleGrad(gSamClampAnisotropic, pos.xy, grad, grad);
}

float4 FxaaTexOff(Texture2D OriginTexture, float2 pos, int2 off)
{
    return OriginTexture.SampleLevel(gSamClampAnisotropic, pos.xy, 0.0, off.xy);
}
...
==============================================================*/
float3 FxaaPixelShader(Texture2D OriginTexture,
...
----------------------------------------------------------------------------*/
    float3 rgbN = FxaaTexOff(OriginTexture,pos.xy, FxaaInt2(0, -1)).xyz;
    float3 rgbW = FxaaTexOff(OriginTexture,pos.xy, FxaaInt2(-1, 0)).xyz;
    float3 rgbM = FxaaTexOff(OriginTexture,pos.xy, FxaaInt2(0, 0)).xyz;
    float3 rgbE = FxaaTexOff(OriginTexture,pos.xy, FxaaInt2(1, 0)).xyz;
    float3 rgbS = FxaaTexOff(OriginTexture, pos.xy, FxaaInt2(0, 1)).xyz;
...
----------------------------------------------------------------------------*/
    float3 rgbNW = FxaaTexOff(OriginTexture,pos.xy, FxaaInt2(-1, -1)).xyz;
    float3 rgbNE = FxaaTexOff(OriginTexture,pos.xy, FxaaInt2(1, -1)).xyz;
    float3 rgbSW = FxaaTexOff(OriginTexture,pos.xy, FxaaInt2(-1, 1)).xyz;
    float3 rgbSE = FxaaTexOff(OriginTexture, pos.xy, FxaaInt2(1, 1)).xyz;
...
--------------------------------------------------------------*/
    ...
    for (int i = 0; i < FXAA_SEARCH_STEPS; i++)
    {
        ...
        if (!doneN)
            lumaEndN =
                FxaaLuma(FxaaTexLod0(OriginTexture,posN.xy).xyz);
        if (!doneP)
            lumaEndP =
                FxaaLuma(FxaaTexLod0(OriginTexture, posP.xy).xyz);
#else
            if(!doneN) lumaEndN = 
...
#endif
    float3 rgbF = FxaaTexLod0(OriginTexture,FxaaFloat2(
