...
class CStray : public CMonster
{
	INFO_CLASS(CStray, CMonster)

protected:
	CStray(ComPtr<ID3D11Device> pDevice, ComPtr<ID3D11DeviceContext> pContext);
	CStray(const CStray& rhs);
	virtual ~CStray() = default;
...
public:
	static shared_ptr<CStray> Create(ComPtr<ID3D11Device> pDevice, ComPtr<ID3D11DeviceContext> pContext);
