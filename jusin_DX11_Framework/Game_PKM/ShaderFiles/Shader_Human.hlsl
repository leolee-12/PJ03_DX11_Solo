...
PS_OUT PS_MAIN(PS_IN In)	// 0번 패스
{
	...
	Out.vAmbt = vector(g_TexAmbt.Sample(LinearSampler, In.vTex).rgb, g_TexSpec.Sample(LinearSampler, In.vTex).r);
	return Out;
}

PS_OUT PS_DA(PS_IN In)	// 2번 패스
{
	PS_OUT Out;
	...
	Out.vNorm = vector(normalize(In.vNorm.xyz) * 0.5f + 0.5f, 1.f);
	Out.vDepth = vector(In.vProjPos.z / In.vProjPos.w, In.vProjPos.w / g_fFarZ, 0.f, 0.f);
	Out.vAmbt = vector(g_TexAmbt.Sample(LinearSampler, In.vTex).rgb, 0.f);
	return Out;
}

PS_OUT PS_DIFF(PS_IN In)	// 3번 패스
{
	PS_OUT Out;
	...
	Out.vNorm = vector(normalize(In.vNorm.xyz) * 0.5f + 0.5f, 1.f);
	Out.vDepth = vector(In.vProjPos.z / In.vProjPos.w, In.vProjPos.w / g_fFarZ, 0.f, 0.f);
	Out.vAmbt = vector(0.f, 0.f, 0.50f, 0.f);
	return Out;
}
...
technique11 DefaultTechnique
{
	...
	pass Pass_DA		// 2. Diff + Ambt
	{
		...
		VertexShader = compile vs_5_0 VS_MAIN();
		GeometryShader = NULL;
		PixelShader = compile ps_5_0 PS_DA();
	}
	pass Pass_Diff		// 3. Diff only
	{
		SetRasterizerState(RS_Default);
		SetDepthStencilState(DSS_Default, 0);
		SetBlendState(BS_Default, float4(0.f, 0.f, 0.f, 0.f), 0xffffffff);
