...
{
	INFO_CLASS(CGraphic_Device, CBase)
...
public:
	/* 그래픽 디바이스의 초기화. */
	HRESULT Initialize(const GRAPHIC_DESC& GraphicDesc, _Inout_ ComPtr<ID3D11Device>* ppDevice,
...
public:
	ID3D11Device* const Get_GraphicDev() const { return m_pDevice.Get(); }
	ID3D11DeviceContext* const Get_GraphicContext() const { return m_pDeviceContext.Get(); }
...
public:
	static CGraphic_Device* Create(const GRAPHIC_DESC& GraphicDesc, _Inout_ ComPtr<ID3D11Device>* ppDevice,
		_Inout_ ComPtr<ID3D11DeviceContext>* ppContext);
