...
CGridCol::CGridCol(LPDIRECT3DDEVICE9 pGraphicDev)
	:CVIBuffer(pGraphicDev), m_hFile(nullptr), m_pPos(nullptr)
{
}

CGridCol::CGridCol(const CGridCol& rhs)
	: CVIBuffer(rhs), m_hFile(rhs.m_hFile), m_pPos(rhs.m_pPos)
{
}
...
HRESULT CGridCol::Ready_Buffer(const _ulong& dwCntX,
	const _ulong& dwCntZ,
	const _ulong& dwVtxItv)
{
	...
	for (_ulong i = 0; i < dwCntZ; ++i)
	{
		for (_ulong j = 0; j < dwCntX; ++j)
		{
			...
			pVertex[dwIndex].vPosition = m_pPos[dwIndex] = { _float(j * dwVtxItv),
...
CGridCol* CGridCol::Create(LPDIRECT3DDEVICE9 pGraphicDev,
	const _ulong& dwCntX, const _ulong& dwCntZ, const _ulong& dwVtxItx)
{
	CGridCol* pGridCol = new CGridCol(pGraphicDev);

	if (FAILED(pGridCol->Ready_Buffer(dwCntX, dwCntZ, dwVtxItx)))
	{
		Safe_Release(pGridCol);
		MSG_BOX("pGridCol Create Failed");
		return nullptr;
	}
