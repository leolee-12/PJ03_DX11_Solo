...
class CMonoDrive : public CMonster
{
	INFO_CLASS(CMonoDrive, CMonster)

protected:
	CMonoDrive(ComPtr<ID3D11Device> pDevice, ComPtr<ID3D11DeviceContext> pContext);
	CMonoDrive(const CMonoDrive& rhs);
	virtual ~CMonoDrive() = default;
...
public:
	static shared_ptr<CMonoDrive> Create(ComPtr<ID3D11Device> pDevice, ComPtr<ID3D11DeviceContext> pContext);
