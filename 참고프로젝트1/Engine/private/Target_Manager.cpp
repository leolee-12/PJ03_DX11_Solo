...
HRESULT CTarget_Manager::Begin_MRT(const _wstring& strMRTTag, _bool isClearTarget, _bool isClearDepth, ID3D11DepthStencilView* pDSV)
{
	...
	/* 동시에 바인딩하는 렌더타겟들은 사이즈가 모두 같ㄷ아야한다. */
	for (auto& pRenderTarget : *pMRTList)
	{
		if(true == isClearTarget)
			pRenderTarget->Clear();
	...
	m_pContext->OMSetRenderTargets(iNumRenderTargets, pRenderTargets, nullptr != pDSV ? pDSV : m_pDSV);
...
HRESULT CTarget_Manager::Copy_RT_Resource(const _wstring& strTargetTag, ID3D11Texture2D* pOut)
{
	CRenderTarget* pRenderTarget = Find_RenderTarget(strTargetTag);
	if (nullptr == pRenderTarget)
		return E_FAIL;
...
HRESULT CTarget_Manager::Render_MRT(const _wstring& strMRTTag, CShader* pShader, CVIBuffer_Rect* pVIBuffer)
{
	list<CRenderTarget*>* pMRTList = Find_MRT(strMRTTag);
	if (nullptr == pMRTList)
		return E_FAIL;
