...
namespace
{
	constexpr const _char* g_szUITypeNames[] =
	{
		"CUIContainer",
		"CUIImage",
		"CUIText",
	...
	constexpr _int g_kDefaultUITypeIndex = 1; // CUIImage
	constexpr _int g_kNumTypes = static_cast<_int>(sizeof(g_szUITypeNames) / sizeof(g_szUITypeNames[0]));

	_int Find_UITypeIndex(const _string& strUITypeName)
	{
		for (_int i = 0; i < g_kNumTypes; ++i)
		{
			if (strUITypeName == g_szUITypeNames[i])
				return i;
		}

		return g_kDefaultUITypeIndex;
	}
}
...
void CPanel_UITool::Draw_UIList()
{
	...
	if (ImGui::Button(KOR("+ UI 요소 추가")))
	{
		...
		newElement.UITypeName = g_szUITypeNames[g_kDefaultUITypeIndex];
		newElement.anchor = "MC";
...
void CPanel_UITool::Draw_ElementProps(UI_ELEMENT& tElement)
{
	...
	// 이름
	char nameBuf[128];
	strncpy_s(nameBuf, tElement.displayName.c_str(), 127);
	if (ImGui::InputText(KOR("이름"), nameBuf, 128))
		tElement.displayName = nameBuf;

	_int iUITypeIndex = Find_UITypeIndex(tElement.UITypeName);
	tElement.UITypeName = g_szUITypeNames[iUITypeIndex];

	if (ImGui::Combo(KOR("UI 타입"), &iUITypeIndex, g_szUITypeNames, g_kNumTypes))
	{
		tElement.UITypeName = g_szUITypeNames[iUITypeIndex];
	}
