						...
						_In_ int       nCmdShow)
{
	...
	CToolApp* pToolApp = CToolApp::Create();
	...
	while (true)
	{
		...
		else
		{
			...
			if (CFrameMgr::GetInstance()->IsPermit_Call(L"Frame60", fTimer_Immediate))
			{
				...
				pToolApp->Update_ToolApp(fTimer_FPS60);
				pToolApp->LateUpdate_ToolApp(fTimer_FPS60);
				pToolApp->Render_ToolApp();
			}
		}
	}
	...
	if (dwRefCnt = Safe_Release(pToolApp))
	{
		MSG_BOX("ToolApp Release Failed");
		return FALSE;
	}
