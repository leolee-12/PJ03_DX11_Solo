...
CState_Aerith_NormalAttack03::CState_Aerith_NormalAttack03(shared_ptr<class CGameObject> pActor, shared_ptr<class CStateMachine> pStatemachine)
	: BASE(pActor, pStatemachine)
{
	...
	if (pModelCom)
	{
		pModelCom->Get_AnimationComponent()
			->Regist_EventToNotify("Battle00|B_NAttack03_0", "Event", "Shoot1",
				MakeDelegate(this, &CState_Aerith_NormalAttack03::Create_Bullet));
		pModelCom->Get_AnimationComponent()
			->Regist_EventToNotify("Battle00|B_NAttack03_0", "Event", "Shoot2",
				MakeDelegate(this, &CState_Aerith_NormalAttack03::Create_Bullet));
	}
}

HRESULT CState_Aerith_NormalAttack03::Initialize_State(CState* pPreviousState)
{
	...
	m_bNextAttack = !m_bPlayable && (STATE_AERITH::eVirtualActionKey == STATE_AERITH::NORMAL_ATTACK4 || STATE_AERITH::eVirtualActionKey == STATE_AERITH::NORMAL_ATTACK5);

	return S_OK;
}

void CState_Aerith_NormalAttack03::Priority_Tick(_cref_time fTimeDelta)
{
	...
	if (m_bNextAttack || (m_bMouseDown && m_pActor_ModelCom.lock()->IsAnimation_Range(34.f, 35.f)))
		m_pStateMachineCom.lock()->Enter_State<CState_Aerith_NormalAttack04>();
	else if (m_pActor_ModelCom.lock()->IsAnimation_Finished())
		m_pStateMachineCom.lock()->Enter_State<CState_Aerith_Idle>();
}

void CState_Aerith_NormalAttack03::Tick(_cref_time fTimeDelta)
{
	__super::Tick(fTimeDelta);
}

void CState_Aerith_NormalAttack03::Late_Tick(_cref_time fTimeDelta)
{
	__super::Late_Tick(fTimeDelta);
}

void CState_Aerith_NormalAttack03::Transition_State(CState* pNextState)
{
...
bool CState_Aerith_NormalAttack03::isValid_NextState(CState* state)
{
	return true;
}

void CState_Aerith_NormalAttack03::Free()
{
}

void CState_Aerith_NormalAttack03::Create_Bullet()
{
	...
	pBullet->Set_StatusComByOwner("Aerith_NormalAttack3");
