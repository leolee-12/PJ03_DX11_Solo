...
HRESULT CRenderer::Initialize()
{
	...
	/* Target_Diffuse */
	if (FAILED(m_pGameInstance->Add_RenderTarget(TEXT("Target_Diffuse"), ViewportDesc.Width, ViewportDesc.Height, DXGI_FORMAT_R8G8B8A8_UNORM, _float4(0.0f, 0.f, 0.f, 0.f))))
		return E_FAIL;
	/* Target_Normal */
	if (FAILED(m_pGameInstance->Add_RenderTarget(TEXT("Target_Normal"), ViewportDesc.Width, ViewportDesc.Height, DXGI_FORMAT_R16G16B16A16_UNORM, _float4(0.0f, 0.f, 0.f, 0.f))))
		return E_FAIL;
	/* Target_Depth */
	if (FAILED(m_pGameInstance->Add_RenderTarget(TEXT("Target_Depth"), ViewportDesc.Width, ViewportDesc.Height, DXGI_FORMAT_R32G32B32A32_FLOAT, _float4(0.0f, 1.0f, 0.f, 0.f))))
		return E_FAIL;
	/* Target_Shade */
	...
	/* Target_World */
	if (FAILED(m_pGameInstance->Add_RenderTarget(TEXT("Target_World"), ViewportDesc.Width, ViewportDesc.Height, DXGI_FORMAT_R32G32B32A32_FLOAT, _float4(0.0f, 0.f, 0.f, 0.f))))
		return E_FAIL;

	/* Target_Final */
	if (FAILED(m_pGameInstance->Add_RenderTarget(TEXT("Target_Final"), ViewportDesc.Width, ViewportDesc.Height, DXGI_FORMAT_R8G8B8A8_UNORM, _float4(0.0f, 0.f, 0.f, 0.f))))
		return E_FAIL;

	/* Target_Blur_X */
	if (FAILED(m_pGameInstance->Add_RenderTarget(TEXT("Target_Blur_X"), ViewportDesc.Width, ViewportDesc.Height, DXGI_FORMAT_R8G8B8A8_UNORM, _float4(0.0f, 0.f, 0.f, 0.f))))
		return E_FAIL;	
	...
	if (FAILED(m_pGameInstance->Ready_RT_Debug(TEXT("Target_Normal"), 100.0f, 300.0f, 200.f, 200.f)))
		return E_FAIL;
	if (FAILED(m_pGameInstance->Ready_RT_Debug(TEXT("Target_Depth"), 100.0f, 500.0f, 200.f, 200.f)))
		return E_FAIL;
	if (FAILED(m_pGameInstance->Ready_RT_Debug(TEXT("Target_Shade"), 350.0f, 150.0f, 300.f, 300.f)))
		return E_FAIL;
	if (FAILED(m_pGameInstance->Ready_RT_Debug(TEXT("Target_Specular"), 350.0f, 450.0f, 300.f, 300.f)))
		return E_FAIL;

	/*if (FAILED(m_pGameInstance->Ready_RT_Debug(TEXT("Target_Shadow"), ViewportDesc.Width - 150.0f, 150.0f, 300.f, 300.f)))
		return E_FAIL;*/

	if (FAILED(m_pGameInstance->Ready_RT_Debug(TEXT("Target_Final"), ViewportDesc.Width - 150.0f, 150.0f, 300.f, 300.f)))
		return E_FAIL;
#endif
...
{
	m_RenderObjects[ENUM_CLASS(eGroupID)].push_back(pRenderObject);
...
void CRenderer::Draw()
{
	...
	/* 지형, 플레이어, 몬스터등을 그리낟. Diffuse + Normal타겟에 그려준다.  */
	Render_NonBlend();
...
{
	if (FAILED(m_pGameInstance->Begin_MRT(TEXT("MRT_Shadow"), true, true, m_pShadowDSV)))
		return;
...
void CRenderer::Render_Lights()
{
	...
	if (FAILED(m_pShader->Bind_Matrix("g_ProjMatrix", &m_ProjMatrix)))
		return;
	if (FAILED(m_pShader->Bind_Matrix("g_ViewMatrixInverse", m_pGameInstance->Get_Transform_Float4x4_Inverse_Ptr(D3DTS::VIEW))))
		return;
	if (FAILED(m_pShader->Bind_Matrix("g_ProjMatrixInverse", m_pGameInstance->Get_Transform_Float4x4_Inverse_Ptr(D3DTS::PROJECTION))))
		return;
...
void CRenderer::Render_Combined()
{
	...
	if (FAILED(m_pShader->Bind_Matrix("g_ProjMatrix", &m_ProjMatrix)))
		return;
	if (FAILED(m_pShader->Bind_Matrix("g_LightViewMatrix", m_pGameInstance->Get_Shadow_Transform_Float4x4_Ptr(D3DTS::VIEW))))		
		return;
	if (FAILED(m_pShader->Bind_Matrix("g_LightProjMatrix", m_pGameInstance->Get_Shadow_Transform_Float4x4_Ptr(D3DTS::PROJECTION))))
		return;
...
void CRenderer::Render_Blur()
{
	...
	if (FAILED(m_pGameInstance->Bind_RT_ShaderResource(TEXT("Target_Blur_X"), m_pShader, "g_BlurXTexture")))
		return;
...
void CRenderer::Render_NonLights()
{
...
void CRenderer::Render_Debug()
{
	...
	if (FAILED(m_pGameInstance->Render_MRT(TEXT("MRT_GameObjects"), m_pShader, m_pVIBuffer)))
		return;
	if (FAILED(m_pGameInstance->Render_MRT(TEXT("MRT_LightAcc"), m_pShader, m_pVIBuffer)))
		return;
	if (FAILED(m_pGameInstance->Render_MRT(TEXT("MRT_Final"), m_pShader, m_pVIBuffer)))
		return;
